...
class Sensor:
    ...
    def get_local_sensed_map(self, occupancy_grid_map: OccupiedGridMap, pos: tuple, direction: float, max_obs_num: int, boundary_obs_list: "list[list[int, int]]", max_attacker_num: int):
        """
        ...
        obstacle_positions = []
        obstacle_adj = np.zeros(shape=(max_obs_num, ))
        attacker_adj = np.zeros(shape=(max_attacker_num, ))
        ...
        for i, beam_direction in enumerate(beam_directions):
            ...
            for index in ray_indices:
                if not occupancy_grid_map.in_bound(index) or occupancy_grid_map.get_map()[index] != 0:
                    ...
                    if occupancy_grid_map.get_map()[index] == 1:
                        idx = boundary_obs_list.index(index)
                        obstacle_adj[idx] = 1
                    else:
                        # TODO:
                        attacker_adj[0] = 1
        ...
        return beam_ranges, obstacle_adj, attacker_adj
