...
class BaseEnv(metaclass=ABCMeta):
    def __init__(self, map_config, env_config, defender_config, attacker_config):
        # Agent config
    ...
    def init_map(self):
        ...
        """
        num = self.map_config.num_obstacle
        center = self.map_config.center
        self.occupied_map = OccupiedGridMap(is3D=self.map_config.is3D, boundaries=self.map_config.map_size)
        self.occupied_map.initailize_obstacle(num=num, center=center)
        inflated_map = deepcopy(self.occupied_map)
        inflated_map.extended_obstacles(extend_range=2)
        return inflated_map
    ...
    def init_attacker(self, inflated_map: OccupiedGridMap, is_percepted: bool, target_list: list):
        ...
        while len(position_list) < self.num_attacker:
            pos = tuple(np.random.normal(loc=pos, scale=5, size=(2,)).clip([0, 0], [width - 1, height - 1]))
            if inflated_map.is_unoccupied(pos=pos):
                if is_percepted:
                    for block in agent_block:
                        dist = np.linalg.norm([block[0] - pos[0], block[1] - pos[1]])
                        if dist < self.defender_config.sensor_range:
                            position_list.append(pos)
